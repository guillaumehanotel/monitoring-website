...
# Renvoie vrai si la date du dernier message envoyé n'existe pas ou date de plus de 2H
def need_recall(last_date):
    need_recall = False
    ...
    if last_date is None:
        ...
        time_since_last_recall = datetime.datetime.now() - last_date

        if time_since_last_recall.seconds//3600 >= 2:
            need_recall = True
...
def check_status():
    with app.app_context():
        db = get_db()
        db.execute('SELECT id, link, warning_message FROM website')
        urls = db.fetchall()
        f = '%Y-%m-%d %H:%M:%S'
        ...
        for url in urls:
            ...
            date_last_warning_message = url[2]

            # si le site est downet si un rappel est nécessaire -> on envoie un message, et on y inscrit la date
            if is_down(id) and need_recall(date_last_warning_message):
                send_message(status_code, link)
                db.execute('UPDATE website SET warning_message = NOW() WHERE id = %(id)s', {'id': id})
